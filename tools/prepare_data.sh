...
if [[ -z "${gpu_python}" || ! -x "${gpu_python}" ]]; then
  echo "GPU Python interpreter not found; set TRAJTC_GPU_PYTHON" >&2
  exit 2
fi
...
"${gpu_python}" "${repo_root}/tools/check_gpu_environment.py"
bash "${repo_root}/tools/get_ocmr.sh" "${data_root}"
mkdir -p "${prepared_root}"
"${gpu_python}" "${repo_root}/tools/check_gpu_environment.py" \
  --output "${prepared_root}/environment.json"
...
export_runtime() {
  ...
  "${gpu_python}" "${repo_root}/experiments/data_prep/export_real_runtime_inputs.py" \
    --case "${case_root}" --frame "${frame}" --measurement-scale 1 \
...
{
  printf 'export TRAJTC_GPU_PYTHON=%q\n' "${gpu_python}"
  printf 'export TRAJTC_DEVELOPMENT_CASE=%q\n' "${cases_root}/fs0152"
  printf 'export TRAJTC_DEVELOPMENT_RUNTIME_ROOT=%q\n' "${development_runtime}"
  printf 'export TRAJTC_HELDOUT_CASES_ROOT=%q\n' "${cases_root}"
  printf 'export TRAJTC_HELDOUT_RUNTIME_ROOT=%q\n' "${heldout_runtime}"
  printf 'export TRAJTC_PERFORMANCE_RUNTIME_ROOT=%q\n' "${development_runtime}"
  printf 'export TRAJTC_PACKED_ROOT=%q\n' "${packed_root}"
  printf 'export TRAJTC_DENSE_ROOT=%q\n' "${dense_root}"
} > "${layout_file}"
