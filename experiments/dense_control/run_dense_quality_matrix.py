#!/usr/bin/env python3
"""Run dense-control eager/Graph CG quality over the frozen 3x3 OCMR matrix."""
...
def main() -> None:
    ...
    with open("/tmp/trajsparsenufft_gpu.lock", "w", encoding="utf-8") as lock:
        ...
        for trajectory in ("spiral", "radial", "golden"):
            ...
            dense_control = dense_root / f"{trajectory}65"
            ...
            for frame in range(3):
                ...
                for graph in (0, 1):
                    ...
                    process = subprocess.run(
                        ...
                        env={
                            **os.environ,
                            "JKF_DENSE_FWD_CONTROL": str(dense_control / "g"),
                            "JKF_DENSE_ADJ_CONTROL": str(dense_control / "gt"),
                        },
                    )
    ...
    summary = {
        ...
        "direct_fp32": {
            "application_passes": sum(
                bool(row["application_vs_fp32_pass"]) for row in rows
            ),
            "worst_complex_rel_l2": max(
            ...
            "minimum_ssim": min(row["native_vs_fp32_ssim"] for row in rows),
            "minimum_psnr_db": min(
